...
var governanceDb = postgres.AddDatabase("governancedb");
...
if (Extensions.IsDataPlatformEnabled(builder.Configuration))
{
    builder.AddDataPlatform(postgres, inventoryApi, governanceDb);
}
...
if (Extensions.IsOrderGeneratorEnabled(builder.Configuration))
{
    ...
    builder.AddProject<Projects.OrderLoadGenerator>("order-load-generator")
        ...
        .WithEnvironment("ORDERGEN_CONTINUOUS", builder.Configuration["OrderGenerator:Continuous"] ?? "true")
        .WithEnvironment("ORDERGEN_VARIABLE_RATE", builder.Configuration["OrderGenerator:VariableRate"] ?? "true")
        .WithEnvironment("ORDERGEN_WAVE_AMPLITUDE_FRACTION", builder.Configuration["OrderGenerator:WaveAmplitudeFraction"] ?? "0.5")
        .WithEnvironment("ORDERGEN_WAVE_PERIOD_SECONDS", builder.Configuration["OrderGenerator:WavePeriodSeconds"] ?? "120")
        .WithEnvironment("ORDERGEN_MINIMUM_RATE_PER_SECOND", builder.Configuration["OrderGenerator:MinimumRatePerSecond"] ?? "5")
        .WithEnvironment("ORDERGEN_MAXIMUM_RATE_PER_SECOND", builder.Configuration["OrderGenerator:MaximumRatePerSecond"] ?? "15")
