...
impl SkuLocationState {
    ...
    pub fn apply_movement(&mut self, fact: &InventoryMovementFact) -> bool {
        ...
        match fact.movement_type {
            InventoryMovementType::Sale => insert_event_time_ordered(&mut self.sales, units),
    ...
    pub fn calculate(&mut self, as_of: DateTime<Utc>) -> Option<SkuLocationFeatures> {
        ...
        if daily_sales > 0 {
            self.model
                .observe_daily_total(as_of.weekday(), daily_sales as f64, as_of);
        }
        if sale_1h > 0 {
            self.model
                .observe_hourly_total(as_of.hour(), sale_1h as f64, as_of);
        }
        let forecast = self
            .model
