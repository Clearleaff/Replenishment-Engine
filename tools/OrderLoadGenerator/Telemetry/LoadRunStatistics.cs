...
public sealed class LoadRunStatistics
{
    ...
    public static string FormatSummary(LoadRunSnapshot snapshot, OrderGeneratorOptions options, int skuCount)
    {
        var text = new StringBuilder()
            .AppendLine("=== ORDER LOAD SUMMARY ===")
            .AppendLine($"Target:             {options.RatePerSecond:F2} req/s for {options.DurationSeconds:F1} s")
            .AppendLine($"Configuration:      profile={options.Profile}, concurrency={options.MaxConcurrency}, seed={options.RandomSeed}, SKUs={skuCount}")
