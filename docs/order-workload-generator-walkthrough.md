...
## Continuous 600 orders/min demo service with wave pacing

The order generator now supports a long-running service mode for replenishment-agent demos.
...
- `RatePerSecond = 10` means the average target is `10 orders/sec`, or `600 orders/min`.
- `Continuous = true` means the generator keeps running cycle after cycle until stopped.
- `VariableRate = true` means the pace rises and falls instead of staying flat.
- `WaveAmplitudeFraction = 0.5` means the rate moves roughly 50% below and above the base rate.
- `WavePeriodSeconds = 120` means one full traffic wave lasts two minutes.
- `MinimumRatePerSecond = 5` caps the low point at `300 orders/min`.
- `MaximumRatePerSecond = 15` caps the high point at `900 orders/min`.

So the demo pattern is:

```text
300 orders/min  →  600 orders/min  →  900 orders/min
       ↓                  ↓                  ↓
low demand       normal demo load      surge/spike load
```
...
```mermaid
flowchart LR
    Timer[Wave-rate scheduler]
    Factory[OrderScenarioFactory]
    Queue[Bounded channel]
    Workers[Concurrent HTTP workers]
    Ordering[Ordering.API]
    Inventory[Inventory.API]
    CDC[Debezium/Kafka/Rust platform]
    Agent[Reorder agent]

    Timer -->|arrival slots| Factory
    Factory -->|random SKU/location/items| Queue
    Queue --> Workers
    Workers -->|POST api/orders| Ordering
    Ordering --> Inventory
    Inventory --> CDC
    CDC --> Agent
```

The generator still uses random order scenarios from `OrderScenarioFactory`: random customers, random order lines, weighted SKU selection, and weighted locations. Profiles like `RegionalSpike` still work and can concentrate demand on SKU 42/NCR.

### AppHost behavior

When `OrderGenerator:Enabled=true`, AppHost now starts the order generator with continuous wave defaults:

```text
ORDERGEN_RATE_PER_SECOND=10
ORDERGEN_CONTINUOUS=true
ORDERGEN_VARIABLE_RATE=true
ORDERGEN_WAVE_AMPLITUDE_FRACTION=0.5
ORDERGEN_WAVE_PERIOD_SECONDS=120
ORDERGEN_MINIMUM_RATE_PER_SECOND=5
ORDERGEN_MAXIMUM_RATE_PER_SECOND=15
```

That gives a live demo feed without manually restarting the generator every minute.

### Manual run command

Use this when the eShop services are already running and you have the correct URLs/secrets:
...
For a sharper reorder-agent demo, use:

```bash
ORDERGEN_PROFILE=REGIONAL_SPIKE ORDERGEN_TARGET_SKU_ID=42
```

That concentrates traffic on SKU 42 at NCR so the feature engine and reorder agent react faster.

### Verification

```bash
dotnet build tools/OrderLoadGenerator/OrderLoadGenerator.csproj --no-restore -v:minimal
dotnet test tests/OrderLoadGenerator.UnitTests/OrderLoadGenerator.UnitTests.csproj --no-restore -v:minimal
```

Result after this checkpoint:

```text
OrderLoadGenerator build: 0 warnings, 0 errors
OrderLoadGenerator tests: 17 passed, 0 failed, 0 skipped
```

`dotnet format --verify-no-changes` reported pre-existing charset/import-order formatting issues in generator files. A broad auto-format was not applied because it would touch multiple existing files beyond this focused change.
